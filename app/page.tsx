...
export default async function Home() {
  ...
  try {
    [featuredProducts, activeProducts, collections] = await Promise.all([getFeaturedProducts(), getActiveProducts(), getCollections()]);
  } catch {
    featuredProducts = [];
    activeProducts = [];
    collections = [];
          ...
          {collections.length === 0 ? (
            ...
            <div className="grid gap-3.5 sm:grid-cols-2 lg:grid-cols-4">
              {collections.slice(0, 4).map((collection, index) => (
                <Link key={collection.id} href={`/collections/${collection.slug}`} className="group relative block overflow-hidden rounded-[14px] border border-[#e5d6c5]">
                  <div className="relative aspect-[4/3]">
                    <Image
                      src={getCollectionPrimaryImage(collection, imagePool[index + 1])}
                      alt={collection.name}
                      fill
                    ...
                    <div className="absolute inset-x-0 bottom-4 px-3 text-center text-[#fbf7f0]">
                      <p className="text-xl font-medium">{collection.name}</p>
                      <p className="mt-1 text-sm text-[#efe4d6]">{collection.description ?? 'تسوقي الآن'}</p>
                    </div>
                  </div>
                </Link>
              ))}
